 ...
 * to.
 *
 * It used to be said twice — `--port "${PORT:-7870}"` on the last line of
 * `run.sh` and `Number(process.env.PORT ?? 7870)` in `register.ts` — with
 * nothing holding the two together, and a third copy of the number sitting in
 * `~/.roadmap/modules` from whenever somebody last ran the second. This is the
 * same shape as the roots `run.sh` argues about further down: a value that lives
 * in two places is a value one edit away from being wrong in one of them, and
 * the failure is quiet either way.
 *
 ...
 * list — and, in this module, walk the papers root — on the way to finding out
 * what to write down.
 *
 * It is a PREFERENCE and not a promise. 7820 through 7960 belong to the other
 * modules on this machine, and if something else holds 7870 when this starts
 ...
 * registry is the thing that has to be true; this number is only where to start
 * looking.
 */
export const PREFERRED_PORT = 7870
